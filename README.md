...
Before running in production, create a `.env` file (copy from `.env.example`) and set **all** of the following:

| Variable | Description |
|---|---|
| `POSTGRES_PASSWORD` | PostgreSQL password. Use a long, random string. **Never use the default.** |
| `POSTGRES_USER` | PostgreSQL username (default: `psono`). Change for production. |
| `POSTGRES_DB` | PostgreSQL database name (default: `psono`). |
| `SECRET_KEY` | Django secret key used for cryptographic signing. **Must be unique per deployment.** Auto-generated by StackBlaze; generate manually with `python -c "import secrets; print(secrets.token_urlsafe(64))"` for self-hosted setups. |

> ⚠️ **The `docker-compose.yml` will refuse to start if `POSTGRES_PASSWORD` is not set.**
> Copy `.env.example` to `.env` and set a strong password before running `docker compose up`.

### Production hardening checklist

- Place Psono behind a TLS-terminating reverse proxy — never expose port 10100 directly to the internet.
- Restrict `ALLOWED_HOSTS` / `ALLOWED_CLIENT_HOSTS` to your domain in the Psono server configuration.
- Review the [Psono server configuration documentation](https://doc.psono.com/admin/installation/install-server-ce.html) for all security-relevant settings.
